...
<section class="mt-8 text-l">
	<div class="container mx-auto px-3">
		<div class="table-box w-10/12 mx-auto">
			...
			<!--  -->
			<a href="/usr/article/write" class="block w-24 h-6 bg-black text-white text-sm text-center rounded mt-4">글쓰기</a>
			<div>
				<!-- 게시판 바깥 공간 시작-->
				...
				<div class="text-sm">
					<form action="list" class="flex items-center gap-2">
						<input type="hidden" name="boardId" value="${boardId }" />
						검색 조건
						<select data-value="${param.searchKeywordTypeCode }" name="searchKeywordTypeCode" class="rounded border border-solid border-gray-400">
							<!-- data-value >>>> data 속성 찾아보기 -->
							<option value="title">제목</option>
							<option value="body">내용</option>
							<option value="title,body">제목+내용</option>
						</select>
						<input value="${param.searchKeyword }" maxlength="20" type="text" name="searchKeyword" class="rounded grow"
							placeholder="검색어를 입력해주세요" />
						<button type="submit" class="w-16 bg-black text-white text-sm rounded">검색</button>
					</form>
				</div>
